...
class DataHelper:
    def __init__(self, config, states):
        ...
        self.input_dir = config['input_dir']  #'data_process/dbook',
    ...
    def load_data(self, state):
        ...
        for supp_x, supp_y, query_x, query_y, social_info in zip(supp_x_list, supp_y_list, query_x_list, query_y_list, social_info_list):
            ...
            self_feat, self_mask = social_info['self_feat'], social_info['self_mask']   #用户自身和对应项目集
            task_self_s.append([self_feat.to(device), self_mask.to(device)])

            social_feat, social_mask = social_info['social_feat'], social_info['social_mask']  #显示朋友

            if 'social_num' in self.config and len(social_feat) > 0:
                social_num = self.config['social_num']  #5
            ...
            if len(social_feat) == 0:
                ...
                social_total_count += len(social_feat[1])  #显示朋友个数

            implicit_feat, implicit_mask = social_info['implicit_feat'], social_info['implicit_mask']  #u-i-a-i-u 隐式朋友
            implicit_num = self.config['implicit_num'] #5
            if len(implicit_feat) > 0:
                implicit_feat = implicit_feat[:, :implicit_num, :, :]   ##没有五个隐式朋友就有几个取几个  超过五个隐式朋友的只取前五个
            ...
            if self.config['use_coclick']:
                coclick_feat, coclick_mask = social_info['coclick_feat'], social_info['coclick_mask']  #用户u-i-u 通过相似性找50隐式朋友
