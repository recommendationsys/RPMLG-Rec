...
def training(model, model_save, model_file_path, data):
    ...
    max_metrics = dict()
    metrics_name = ['mae', 'rmse', 'ndcg@5', 'Recall@5']
    for state in states:
        if state != 'meta_training':
            max_metrics[state] = [100., 100., 0, 0]
    ...
    for epoch in range(num_epoch):
        ...
        for k in range(num_batch):  # each batch contains some tasks (each task contains a support set and a query set)
            batch_data = data.get_batch('meta_training', k)  # 取一批数据
...
def testing(epoch, model, data, metrics_dict):
    ...
    for state in states:
        if state == 'meta_training':
            continue
        loss, mae, rmse, ndcg_at_5, Recall_5 = [], [], [], [], []
        ...
        for j in range(len(test_data['supp_xs'])):  # each task
            ...
            _loss, _mae, _rmse, _ndcg_5, _Recall_5 = model.evaluation(task_data)
            loss.append(_loss)
            mae.append(_mae)
            rmse.append(_rmse)
            ndcg_at_5.append(_ndcg_5)
            Recall_5.append(_Recall_5)
        loss_ = torch.stack(loss).mean(0)
        loss_ = loss_.cpu().data.numpy()
        mae_mean = np.mean(mae)
        rmse_mean = np.mean(rmse)
        ndcg_mean = np.mean(ndcg_at_5)
        Recall_mean = np.mean(Recall_5)
        ...
        model.writer.add_scalar('epoch_test_%s_Recall_5' % state, Recall_mean, global_step=epoch)
        output_to_file('{}: state: {}; loss: {:.5f},mae: {:.5f}, rmse: {:.5f}, ndcg@5: {:.5f}, Recall@5: {:.5f}'.
                       format(get_current_time(), state, loss_, mae_mean, rmse_mean, ndcg_mean, Recall_mean), log_file)
        ...
        if ndcg_mean > metrics_dict[state][2]:
            ...
            update = True
        if Recall_mean > metrics_dict[state][3]:
            metrics_dict[state][3] = Recall_mean
            update = True
    return update
...
if __name__ == "__main__":
    ...
    load_model = False   # 作者load_model = True
