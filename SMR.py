...
class SMR(torch.nn.Module):
    ...
    def local_update(self, task_data):
        ...
        support_user_emb = self.user_emb(support_x[:, self.config['num_fea_item']:])  #'num_fea_item': 2,
        ...
        task_embd_dict['self_users_embd'], task_embd_dict['self_items_embd'] = self.get_user_item_embedding(task_self_feat) #自身
        ...
        task_embd_dict['social_users_embd'], task_embd_dict['social_items_embd'] = self.get_user_item_embedding(task_social_feat)  #显示朋友
        ...
        task_embd_dict['implicit_users_embd'], task_embd_dict['implicit_items_embd'] = self.get_user_item_embedding(task_implicit_feat)  #u-i-a-i-u  （只有5个）
        ...
        if self.config['use_coclick']:
            ...
            task_embd_dict['coclick_users_embd'], task_embd_dict['coclick_items_embd'] = self.get_user_item_embedding(task_coclick_feat) #u-i-u  （最多只有5个）
            task_embd_dict['coclick_mask'] = task_coclick_mask


        task_weights = self.meta_learner.update_parameters()


        support_pred,trpn_loss = self.meta_learner(support_item_emb, support_user_emb, vars_dict=task_weights, **task_embd_dict)
        loss = F.mse_loss(support_pred, support_y)+trpn_loss
        ...
        for _ in range(1, self.config['local_update']):   #'local_update': 1  ##不运行
            support_pred, trpn_loss = self.meta_learner(support_item_emb, support_user_emb, vars_dict=fast_ml_weights, **task_embd_dict)
            loss = F.mse_loss(support_pred, support_y) + trpn_loss
        ...
        # start_time = time.time()
        # query_pred, trpn_loss = self.meta_learner(query_item_emb, query_user_emb, vars_dict=fast_ml_weights, **task_embd_dict)

        query_pred, trpn_loss = self.meta_learner(query_item_emb, query_user_emb, vars_dict=task_weights, **task_embd_dict)
        loss = F.mse_loss(query_pred, query_y)+trpn_loss
    ...
    def evaluation(self, task_data):
        ...
        Recall_5 = self.cal_metrics.Recall_at_k(self.query_y_real, self.query_y_pred, k=5)
        return loss, mae, rmse, ndcg_5 ,Recall_5
