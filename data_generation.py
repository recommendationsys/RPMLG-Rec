...
def get_coclick_neighbor(iids, uid_iids_map, iid_uids_map, max_num):  #以第一个用户为例 讲解
...
def get_index_mapping(dataset, output_dir):
    ...
    # print("####item_set#####")
    # print(item_set)
    # print("####user_feat_name_list#####")
    # print(user_feat_name_list)
    # print("####item_feat_name_list#####")
    # print(item_feat_name_list)
...
def get_task_tensor(uid_list, feats_list, labels_list, all_label_list):
    ...
    # print("feat_mask_tensor",feat_mask_tensor)
...
def get_training_samples(dataset_path, user2id, item2id, user_feature, item_feature, output_dir, **kwargs):
    ...
    for state in states:
        ...
        for _, (user, items) in tqdm(enumerate(dataset.items())):
            ...
            if item_len < 13 or item_len > 100:
                continue
    ...
    print("len(user_num)",len(user_num))  # 6576
    print("len(item_num)",len(item_num))  # 20547
    print("rating_num",rating_num)  # 326419
...
def process(data_set, item_feat_list, implicit_num):
    ...
    # construct mapping between user, item, features and index   这些关系都是来自original中，应该是包含所有数据
    ...
    other_data = {'uid_iids_map': uid_iids_map,
                  ...
                  'item_feat_list_for_graph': item_feat_list,  #['publisher']
